...
`task/basic.md` 中每个 `### T-B<n>` 的四项：结构改动、判定依据、被哪些 detail 任务依赖、design.md 中的出处（判定见 `rules.md` §4、§6）。

`task/detail.md` 中每个 `### T-D<n>` 的三项：行为目标、对应 `specify.md` 条目、依赖的基础任务——后两项分别是 trace 链与 `rules.md` §8 消费者关系在本文件的落点。
...
引用 `SPEC-` 与 `T-D` 编号时的三种写法，均来自已完成实例的实际书写：
...
上表第二、三行里的裸数字（`004`、`005`）紧跟在完整编号之后——**写法的判定与展开规则见 `rules.md` §12**。

## 断言形态

工件里的数字与存在性断言记明它是哪条命令跑出来的——**该要求与其判定者见 `rules.md` §7**，本节只给记法示例。
