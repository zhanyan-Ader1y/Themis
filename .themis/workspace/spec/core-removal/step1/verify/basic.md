# verify/basic.md — core-removal / step1
...
**本文件不构成任何下游节点的通过凭据。** 按 `flow.md`「impl/detail」节的前置闸门条款，本 step 的 impl/detail 在空段下不以本文件的结论为前置。也就是说：本结论悬空，不阻塞、也不解锁下一节点——这一点由控制面本身规定，不是本节点的裁量。**但这不等于下一节点已获放行**：impl/detail 能否开始另有一处互斥读法未决，见「说明」第 2 点与末段；本节点不代为落定，`state.md`「当前节点」据此停在本节点之后、等待裁定。
...
**fail-closed 是否适用，控制面同样没说。** `rules.md` §10 列举了触发停靠的几种情形，"结论不可判"不在其列，也没有条款说它算不算。本节点按"不在其列"处理，即不触发停靠；但这是读法，不是条款。若所有者认为不可判应当等同于失败，本 step 应停回 R3 那道已证闸门，任务 6 不得开始——这一条留给所有者裁定，本节点不自行落定。
