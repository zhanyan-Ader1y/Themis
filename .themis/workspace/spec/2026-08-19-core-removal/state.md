# state.md — 2026-08-19-core-removal
...
**当前节点：无——本 spec 实例已闭合，需求达成。** `Intent.md`「step 分解」现只载 step1 一项，已 accepted。step2 曾于 2026-08-24 新增、2026-08-26 由所有者撤销，只进入过 step 定界节点、未产出任何工件（经过见 `intent-review.md`「结论再追加：step2 撤销」）。

**撤销不使任何已有工件失效**：撤销的是一个从未产出工件的分解项，step1 的目标与批准前提一字未改。
...
### ~~step2~~（2026-08-26 撤销，无闸门记录）

step2 由所有者撤销，不再是本需求的分解项。它**只进入过 step 定界节点且未产出任何工件**——执行者在该节点核实目标时发现范围大于分解项，按 `flow.md`「step 定界」节停下提问，随后所有者撤销。因此本组无闸门可记，也无 `step2/` 目录。

撤销理由与所有者原话见 `intent-review.md`「结论再追加：step2 撤销」；分解项的撤销记述见 `Intent.md`「step 分解」。
...
- `Intent.md`（2026-08-24 追加、2026-08-26 再追加）：仍为 current。2026-08-24 新增「step 分解」小节；2026-08-26 依所有者答复撤销其中的 step2 项，需求范围随之收窄为"删除 `core/` 本体与其活跃引用"。两次修订按 `flow.md`「R1 意图评审」节的失效级联例外**均不触发 stale**——该例外覆盖"不改变任何已有 step 目标语义"的分解修订，两次都落在此范围内：step1 的目标、期望结果、核心链路、范围与非做始终一字未改。分解的演进在该小节如实记录，不回填改写。
- `intent-review.md`（2026-08-26 两次追加）：current。先记 step 分解确认，后记 step2 撤销；前者为当时真实发生的确认，逐字保留不改写。两次追加均只补记评审者本人的答复，不超出其内容。
