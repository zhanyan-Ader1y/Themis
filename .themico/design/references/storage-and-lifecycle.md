...
- 单个 L3 Markdown 当前实现上限为 128 KiB（由 `query.Inspect` depth=3 读取时 canonical 的 1 MiB machine JSON envelope 硬上限反推得出；这条上限只关闭 L3 内容本身导致的读不回，不构成"任何已发布记录都能被 depth-3 读回"的整体保证——`L1`/`L2`/`Scope` 缺少独立字节上限的缺口仍然存在；恢复到接近本节其余各项所暗示的更大数量级，需要先解决该 envelope 预算模型本身并一并补上 L1/L2/Scope 的独立上限，属后续独立计划）；
