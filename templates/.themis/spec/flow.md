...
| 3 | 意图定稿 | Why、期望结果、核心链路三者均明确 | `Intent.md` | step 拆分及其后全部 | 停在追问 |
| 4 | step 拆分 | 意图定稿完成 | `step.md`；各 `step<N>/` 目录 | R1 及其后全部 | 停在意图定稿 |
| 5 | R1 意图评审 | 意图定稿完成且 step 拆分成形 | `intent-review.md` | 抽象设计及其后全部 | 停在 step 拆分 |
...
5. **状态落盘，续接不靠推断**：每个节点结束时必须更新 `state.md`，记录当前节点、各闸门的 pass/fail 与当前性（currentness）。中断后的续接以 `state.md` 与已物化工件为准，**不从对话历史、Agent 自述或文件存在推断进度**。此条对应已批准的 `SPEC-ENFORCE-001`（soft 执行器须以人类可读状态文件记录当前节点、各评审 pass/fail 与 currentness）。
