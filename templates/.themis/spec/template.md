...
上表每份工件的小节划分，就是 `rules.md` §9 要求的控制事实与人类语义配比的落地形态：每份工件必须同时承载可核验的控制事实小节与承载说明、理由的人类语义小节。§9 是贯穿全部产出工件节点的通用规则，不依附任何单一节点，因此不会被 `flow.md` 的某个节点专门引用——它正是通过本文件逐份工件的小节划分获得落点。
