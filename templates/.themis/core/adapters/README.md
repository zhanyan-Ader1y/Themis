...
输入来自 `workspace/project.md` 中显式配置的 adapter 和 credentials boundary；输出必须转换为 owning protocol 的结构化 observation/evidence，并保留原始来源引用。
...
该 package 当前没有实现资产。`workspace/project.md` 只有空 `adapters` 配置位；没有 adapter protocol、runtime 或 tests，也没有活动核心计划依赖它。
