...
P5.2 已按本设计完成：原生 Artifact v2 / Spec v2 协议、双视图执行器、Specification/Planning 契约、模板检查和回归矩阵均已落地。不存在 Spec v1 运行时兼容或 Artifact v1→v2 迁移能力。P8 边界保持不变，当前不持久化 `draft → specified`。

已观察的验证结果：

- Spec Artifact：30/30；
- Template Contract：58/58；
- Init：30/30；
- Upgrade：37/37；
- Migration：52/52；
- Init Environment：31/31；
