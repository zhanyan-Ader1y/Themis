...
`## §8 孤儿阻断`
- 适用节点：verify/detail（判定发生处）、人工验收（据结论阻断）。
- 判据：不存在"已落地但无消费者"的 basic 改动。每个 basic 任务都能指出被哪些 detail 任务依赖，且其消费者在实际代码中存在。
- 拒绝条件：存在无消费者的 basic 改动而进入人工验收；以"将来会用到"作为消费者；以删除记录代替显式重规划处理；把判定推给没有代码证据的验收者。
- 判定者：**detail 段验证角色**做判定并把结论写入 `verify/detail.md`；**验收节点的人类验收者**在 `acceptance.md` 的阻断核查中引用该结论、据以阻断。

**孤儿判定在 detail 段验证完成，验收只引用结论。** 这是主仓 `488ef51`（"孤儿判定移入 detail 段验证，验收只引用结论"）已合入的设计决定：判定需要代码证据，而验收者手上没有。`SPEC-ACCEPT-002` 只规定"存在孤儿时不得进入人工验收"这一阻断效果，未指定判定发生在哪一步，两者相容。
