...
**`README.md` 不在控制链路上。** 它是说明性文档,供人了解运行机制,不被流程加载、不承载任何运行时权威。因此凡运行时必须被读到的内容(当前强制水平、安全降级)必须同时写在 `SKILL.md` 正文里,不能只放在 README。
...
| `README.md` | 向人解释这套东西怎么运行（说明性,不在控制链路上,不含任何流程或判定内容） |
